...
###################################################################################################
def detectCharsInPlates(listOfPossiblePlates, recognizedPlates):
    intPlateCounter = 0
    imgContours = None
    contours = []
    recognizedPlate = False

    if len(listOfPossiblePlates) == 0:          # if list of possible plates is empty
        return listOfPossiblePlates, recognizedPlate, recognizedPlates             # return
    # end if
    ...
    for possiblePlate in listOfPossiblePlates:          # for each possible plate, this is a big for loop that takes up most of the function
        ...
        possiblePlate.imgGrayscale, possiblePlate.imgThresh = Preprocess.preprocessPlate(possiblePlate.imgPlate)     # preprocess to get grayscale and threshold images

        if Main.showSteps == True: # show steps ###################################################
            cv2.imshow("Posible plate", possiblePlate.imgPlate)
            cv2.imshow("Posible plate gray scale", possiblePlate.imgGrayscale)
            cv2.imshow("Posible plate threshold", possiblePlate.imgThresh)
        # end if # show steps #####################################################################
        ...
        if Main.showSteps == True: # show steps ###################################################
            cv2.imshow("Posible plate bigger", possiblePlate.imgThresh)
        # end if # show steps #####################################################################
        ...
        if Main.showSteps == True: # show steps ###################################################
            ...
            cv2.imshow("Posible chars in plate", imgContours)
        # end if # show steps #####################################################################
        ...
        if Main.showSteps == True: # show steps ###################################################
            ...
            for listOfMatchingChars in listOfListsOfMatchingCharsInPlate:
                ...
                cv2.drawContours(imgContours, contours, -1, (intRandomBlue, intRandomGreen, intRandomRed))
            # end for
            cv2.imshow("Countours found without overlap", imgContours)
        # end if # show steps #####################################################################
        ...
        if Main.showSteps == True: # show steps ###################################################
            ...
            cv2.imshow("Best Match", imgContours)
        # end if # show steps #####################################################################

        possiblePlate.strChars = recognizeCharsInPlate(possiblePlate.imgThresh, longestListOfMatchingCharsInPlate)
        if(possiblePlate.strChars in recognizedPlates):
            print("LICENSE ALREADY SAVED")
            recognizedPlate = True
        else:
            recognizedPlates.append(possiblePlate.strChars)
            recognizedPlate = False
    ...
    return listOfPossiblePlates, recognizedPlate, recognizedPlates
# end function
...
def checkIfPossibleChar(possibleChar):
    ...
    return(possibleChar.intBoundingRectArea > MIN_PIXEL_AREA and possibleChar.intBoundingRectWidth > MIN_PIXEL_WIDTH 
        and possibleChar.intBoundingRectHeight > MIN_PIXEL_HEIGHT and MIN_ASPECT_RATIO < possibleChar.fltAspectRatio 
...
def recognizeCharsInPlate(imgThresh, listOfMatchingChars):
    ...
    if Main.showSteps == True: # show steps #######################################################
        cv2.imshow("Final plate", imgThreshColor)
    # end if # show steps #########################################################################
