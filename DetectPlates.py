...
def detectPlatesInScene(imgOriginalScene):
    ...
    if Main.showSteps == True: # show steps #######################################################
        cv2.imshow("Original frame", imgOriginalScene)
        waitOrExit()
    # end if # show steps #########################################################################
    ...
    if Main.showSteps == True: # show steps #######################################################
        cv2.imshow("OF GrayScale", imgGrayscaleScene)
        cv2.imshow("OF ThreshHold", imgThreshScene)
        waitOrExit()
    # end if # show steps #########################################################################
    ...
    if Main.showSteps == True: # show steps #######################################################
        print("List of matching chars = " + str(
        ...
        cv2.imshow("Countours detected", imgContours)
    # end if # show steps #########################################################################
    ...
    if Main.showSteps == True: # show steps #######################################################
        ...
        for i in range(0, len(listOfPossiblePlates)):
            ...
            cv2.imshow("Posibles plates", listOfPossiblePlates[i].imgPlate)
            cv2.waitKey(0)
        # end for
...
def findPossibleCharsInScene(imgThresh):
    ...
    _, contours, _ = cv2.findContours(imgThreshCopy, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)   # find all contours
    ...
    for i in range(0, len(contours)):                       # for each contour
        ...
        if DetectChars.checkIfPossibleChar(possibleChar):                   # if contour is a possible char, note this does not compare to other chars (yet) . . .
            ...
            if Main.showSteps == True: # show steps #######################################################
                cv2.drawContours(imgContours2, contours, i, Main.SCALAR_WHITE)
                cv2.imshow("Countours found", imgContours2)
                # waitOrExit()
        # end if
    # end for

    if Main.showSteps == True: # show steps #######################################################
        print("\nCountours found: " + str(len(contours)))  # 2362 with MCLRNF1 image
        print("\nChars detected = " + str(intCountOfPossibleChars))  # 131 with MCLRNF1 image
        cv2.imshow("Contours in image", imgContours)
        waitOrExit()
    # end if # show steps #########################################################################
